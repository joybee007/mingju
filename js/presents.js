$(function () {
	...
	function showPop(i) {
		...
		showImg($pop.find('.popLi:eq(' + i + ')').find('.pop-img'), 0);
	}
	...
	function showImg(ul, i) {
		...
		if (i === 0) {
			$pop.find('.pop-prev').hide();
		} else if (i === list.length - 1) {
			$pop.find('.pop-next').hide();
		}
	...
	$pop.find('.pop-next').click(function () {
		var ul = $pop.find('.popLi:visible .pop-img ul'),
			ci = $pop.find('.popLi:visible .pop-img ul li:visible').index();
		showImg(ul, ci + 1);
	});
	$pop.find('.pop-prev').click(function () {
		var ul = $pop.find('.popLi:visible .pop-img ul'),
			ci = $pop.find('.popLi:visible .pop-img ul li:visible').index();
		showImg(ul, ci - 1);
	});
	$pop.find('.btns div').hover(function () {
		$(this).addClass('hover');
	}, function () {
